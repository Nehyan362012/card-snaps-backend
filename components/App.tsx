...
import { CardyPage } from './CardyPage';
...
import { SnapCardsPage } from './SnapCardsPage';
...
const App: React.FC = () => {
          ...
          if (importDeck) {
              try {
                  ...
                  if (deckData.cards && Array.isArray(deckData.cards)) {
                      ...
                      setDecks((prev: Deck[]) => [savedDeck, ...prev]);
                      
                      await api.shareToCommunity(savedDeck, 'deck', 'Shared Link');
                      
                      soundService.playSuccess();
                      alert(`Deck "${savedDeck.title}" imported successfully!`);
  ...
  // Removed separate auth check - now handled in ProfileSetup
  useEffect(() => {
      // Just set loading to false - auth will be handled in ProfileSetup
      setAppLoading(false);
  }, []);
  ...
  const handleDeleteDeck = async (e: React.MouseEvent, deckId: string) => {
    ...
    if (confirm("Are you sure you want to delete this deck?")) {
      ...
      setDecks((prev: Deck[]) => prev.filter(d => d.id !== deckId));
    }
  };
  ...
  const handleImportItem = (item: Deck | Note, type: 'deck' | 'note') => {
      if (type === 'deck') {
          setDecks((prev: Deck[]) => [item as Deck, ...prev]);
          api.createDeck(item as Deck);
      } else {
          setNotes((prev: Note[]) => [item as Note, ...prev]);
  ...
  const renderContent = () => {
      ...
      if (view === AppView.DOCK) {
          return <DockPage 
                    onDeckAdded={(d: Deck) => setDecks((prev: Deck[]) => [d, ...prev])} 
                    onNoteAdded={(n: Note) => setNotes((prev: Note[]) => [n, ...prev])} 
                 />;
      }
